...
T lmcf_cost_scaling(
    ...
    lemon::CostScaling<lemon::ListDigraph, T, T> solver(graph);
    ...
    solver.supplyMap(node_supply_map);
    auto status = solver.run();
    if (status != lemon::CostScaling<lemon::ListDigraph, T, T>::OPTIMAL) {
        if (status == lemon::CostScaling<lemon::ListDigraph, T, T>::INFEASIBLE)
            throw std::runtime_error("Solver failed: problem is INFEASIBLE");
        else if (status == lemon::CostScaling<lemon::ListDigraph, T, T>::UNBOUNDED)
            throw std::runtime_error("Solver failed: problem is UNBOUNDED");
        else
            throw std::runtime_error("Solver failed with unknown status");
    }
...
// Cost-scaling overload — no lower bounds
template <typename T>
T lmcf_cost_scaling(
    std::span<T> node_supply,
    std::span<T> edges_starts,
    ...
    requires std::is_signed<T>::value && std::is_integral<T>::value
{
    return lmcf_cost_scaling(node_supply, edges_starts, edges_ends, capacities, std::span<T>{}, costs, result);
}
...
T lmcf_capacity_scaling(
    ...
    lemon::CapacityScaling<lemon::ListDigraph, T, T> solver(graph);
    ...
    solver.supplyMap(node_supply_map);
    auto status = solver.run();
    if (status != lemon::CapacityScaling<lemon::ListDigraph, T, T>::OPTIMAL) {
        if (status == lemon::CapacityScaling<lemon::ListDigraph, T, T>::INFEASIBLE)
            throw std::runtime_error("Solver failed: problem is INFEASIBLE");
        else if (status == lemon::CapacityScaling<lemon::ListDigraph, T, T>::UNBOUNDED)
            throw std::runtime_error("Solver failed: problem is UNBOUNDED");
        else
            throw std::runtime_error("Solver failed with unknown status");
    }
...
// Capacity-scaling overload — no lower bounds
template <typename T>
T lmcf_capacity_scaling(
    std::span<T> node_supply,
    std::span<T> edges_starts,
    ...
    requires std::is_signed<T>::value && std::is_integral<T>::value
{
    return lmcf_capacity_scaling(node_supply, edges_starts, edges_ends, capacities, std::span<T>{}, costs, result);
}
