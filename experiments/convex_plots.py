from pylmcf import wasserstein_integer, Spectrum, WassersteinSolver, SimpleTrash, wasserstein_integer_compat
...
def plot_convex():
    ...
    solver = WassersteinSolver(Sp1, [Sp2, Sp3], trashes=[SimpleTrash(trash_cost)], intensity_scaling=SCALE, costs_scaling=SCALE)
...
def plot_3d_convex():
    ...
    solver = WassersteinSolver(Sp1, [Sp2, Sp3, Sp4], trashes=[SimpleTrash(trash_cost)], intensity_scaling=1, costs_scaling=1)
    ...
    for fraction1 in tqdm(params1):
        for fraction2 in params2:
            ...
            #     continue
            intensities = np.concatenate((intensities1 * fraction1, intensities2 * fraction2, intensities3 * (2.0 - fraction1 - fraction2)))
            X = np.concatenate((X1, X2, X3))
            Y = np.concatenate((Y1, Y2, Y3))
            res1 = wasserstein_integer_compat(X, Y, intensities, X4, Y4, intensities4, trash_cost)
            outs1.append(res1['total_cost'])
            res2 = wasserstein_integer(X, Y, intensities, X4, Y4, intensities4, trash_cost)
...
def plot_3d_convex():
    ...
    Sp3 = Spectrum(np.array([X3, Y3]), intensities3)
    Sp4 = Spectrum(np.array([X4, Y4]), intensities4)
    solver = WassersteinSolver(Sp4, [Sp1, Sp2, Sp3], trashes=[SimpleTrash(trash_cost)], intensity_scaling=1, costs_scaling=1)
...
def plot_nonscaled():
    ...
    Sp2 = Spectrum(np.array([X2, Y2]), intensities2)
    Sp4 = Spectrum(np.array([X4, Y4]), intensities4)
    solver = WassersteinSolver(Sp4, [Sp1, Sp2], trashes=[SimpleTrash(trash_cost)], intensity_scaling=1, costs_scaling=1)
    ...
    for fraction1 in tqdm(params1):
        for fraction2 in params2:
            # if fraction1 + fraction2 > 1:
            #     continue
            intensities = np.concatenate((intensities1 * fraction1, intensities2 * fraction2))
            X = np.concatenate((X1, X2))
            Y = np.concatenate((Y1, Y2))
            res1 = wasserstein_integer_compat(X, Y, intensities, X4, Y4, intensities4, trash_cost)
            outs1.append(res1['total_cost'])
...
def plot_masserstein():
    from masserstein_example import convolved, spectra, masserstein_convolved, masserstein_spectra
    solver = WassersteinSolver(convolved, spectra, trashes=[SimpleTrash(0.05)], intensity_scaling=10000000, costs_scaling=10000000)
    ...
    import masserstein
    mp = masserstein.estimate_proportions(masserstein_convolved, masserstein_spectra, MTD=0.05)['proportions']
    mp = np.array(mp)
