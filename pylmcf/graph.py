...
class Graph:
    ...
    def build(self):
        ...
        self.lemon_graph = pylmcf_cpp.LemonGraph(self.no_nodes, self.edge_starts, self.edge_ends, self.edge_costs)

    def set_edge_capacities(self, edge_ids, capacities):
        assert self.order is not None, "Cannot set edge capacities before building the graph"
    ...
    def __str__(self):
        return "Graph with %d nodes and %d edges" % (self.no_nodes, len(self.edge_starts))

    def print(self):
        print(str(self))
        print("Edges:")
        for i in range(len(self.edge_starts)):
            print("Edge %d: %d -> %d, cost %d, capacity %d, flow %d" % (i, self.edge_starts[i], self.edge_ends[i], self.edge_costs[i], self.edge_capacities[i], self.flows[i]))