...
class SimpleTrash(Trash):
    ...
    def add_to_network(self, WNM):
        self.G = WNM.G
        self.WNM = WNM
        self.trash_edge = self.G.add_edge(
            self.WNM.source, self.WNM.sink, np.int64(self.trash_cost * self.scale)
        )
...
class EmpiricalTrash(Trash):
    ...
    def add_to_network(self, WNM):
        ...
        self.trash_edges = self.G.add_edges(
            edge_starts,
            edge_ends,
            np.full(len(edge_starts), self.trash_cost * self.scale),
        )
...
class TheoryTrash(Trash):
    ...
    def add_to_network(self, WNM):
        ...
        self.self_single_theory_trashes = [
            SingleTheoryTrash(WNM, self.trash_cost * self.scale, STM)
            for STM in self.matching_network.theory_matchers
        ]
